...
export class CartService {
  ...
  async addToCart(userId: string, addDto: AddToCartDto) {
    ...
    return { message: 'Đã thêm vào giỏ hàng', cart: await this.cartRepository.findByUserId(userId) };
  }
  ...
  async updateCartItem(userId: string, productId: string, updateDto: UpdateCartItemDto) {
    ...
    if (itemIndex > -1) {
      ...
      await this.cartRepository.save(cart);
      return { message: 'Đã cập nhật số lượng', cart: await this.cartRepository.findByUserId(userId) };
    } else {
  ...
  async removeCartItem(userId: string, productId: string) {
    ...
    return { message: 'Đã xoá sản phẩm khỏi giỏ hàng', cart: await this.cartRepository.findByUserId(userId) };
  }
